...
void	ft_putstr(datas *params, int **answer)
{
	...
	while (line < params->val_max) // tant que j'ai pas ecrit ma derniere ligne
	{
		while (box < params->val_max)
			// tant que je suis pas arriver a la derniere case de chaque ligne
		{
			if (box == 0) // si je suis a la premiere case,
				j'ecris la conddtion corresponndante
...
int	ft_recursive_verif(datas *params, int **answer, int line)
{
	...
	reset = answer[line];
	cmb_nb = ft_recursive_factorial(params->val_max) - 1;          
		// je pars de la derniere combinaison
	answer[line] = find_comb(params, &cmb_nb, reset, line, answer);
		// je trouve la premiere ligne correcte
	result = 0;
	while (line < params->val_max)
		// tant que j'ai pas atteint mon nombre de lignes max
	{
		if (answer[line][0] != 0 && check_lines_bloc(params, answer, line))
			// je test ma valeur avec les valeurs precedentes
		{                                                                  
			// si c'est bon
				...
				// si j'arrive a la derniere case validee
			{
				...
				result += ft_recursive_verif(params, answer, line + 1);
...
int	rush01(char **av, int val_max)
{
	...
	params->combs = all_possibilities(1, val_max);  
		// j'alloue ma grille de reponses
	params->conditions = ft_write_cond(av, val_max);
		// j'assigne ma grille de conditions
	answer = malloc(sizeof(int *) * val_max);       
		// j'alloue ma grille de reponses
