...
int	ft_recursive_verif(int **answer, int val_max, int *conds)
{
	...
	box = 0;
	i = answer[val_max + 1][box];//je prends ma valeur de i a la case actuelle
	while (box < (val_max * val_max))//tant que j'ai pas atteint mon nombre de box max
	{
		if (answer[0][box] != 0)//si j'avais deja une valeur
		{
			...
			else if (start_check(conds, answer, box, val_max))//si j'ai une valeur coorrecte
			{
				mini_pre_filling(conds, answer, val_max, box);
				box++;
		...
		{
			while (i >= 0)//tant que j'ai pas tester toutes les valeurs valides
			{
				while (answer[i][box] == 0 && i > 0)
					i--;
				answer[val_max + 1][box] = i;//j'assigne ma nouvelle valeur actuelle a la case correspondante
				answer[0][box] = i--;//j'assigne ma valeur
				if (start_check(conds, answer, box, val_max) && answer[0][box] != 0)//et je commence le check
				{//si c'est bon
					if (box == (val_max * val_max - 1))//si j'arrive a la derniere case validee
...
int	rush01(char **av, int val_max)
{
	...
	if (cond_peers_validity(conds, val_max))
		i = ft_recursive_verif(answer, val_max, conds);//je verifie les valeurs
	else
		error_peers();
