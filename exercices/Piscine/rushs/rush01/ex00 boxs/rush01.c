...
void	ft_write_cond(int *conds, char **av, int val_max)
{
	...
	i = 0;
	j = 0;
	while (av[1][i] && j < val_max * 4)
		// je convertis les chars en int en skippant les white spaces
	{
		conds[j++] = av[1][i] - '0';
		i += 2;
	}
...
/* int	ft_recursive_verif(int **answer, int box, int val_max, int *conds)
{
	...
	{
		while (box < (val_max * val_max)
			&& i > 0)//tant que j'ai pas atteint mon nombre de box max
		{
			...
			if (start_check(conds, answer, box, val_max)
				&& answer[0][box] != 0)//et je commence le check
			{//si c'est bon
				if (box == (val_max * val_max
						- 1))//si j'arrive a la derniere case validee
				{
					ft_putstr(answer, val_max, conds);//j'ecris la reponse
					result++;
				}
				else
					result += ft_recursive_verif(answer, box + 1, val_max,
							conds);//je vais a la prochaine case
			}
			if (result == 1)
				break ;
			answer[0][box] = 0;
...
{
	box = 0;
	i = answer[val_max + 1][box];    
		// je prends ma valeur de i a la case actuelle
	while (box < (val_max * val_max))
		// tant que j'ai pas atteint mon nombre de box max
	{
		if (answer[0][box] != 0) // si j'avais deja une valeur
		{
			if (box == (val_max * val_max - 1))
			{
				ft_putstr(answer, val_max, conds); // j'ecris la reponse
		...
		{
			while (i >= 0)
				// tant que j'ai pas tester toutes les valeurs valides
			{
				while (answer[i][box] == 0 && i > 0)
					i--;
				answer[val_max
					+ 1][box] = i;                                       
					// j'assigne ma nouvelle valeur actuelle a la case correspondante
				answer[0][box] = i--;                                               
					// j'assigne ma valeur
				if (start_check(conds, answer, box, val_max)
					&& answer[0][box] != 0) // et je commence le check
				{                                                                   
					...
					if (box == (val_max * val_max
							- 1))                             
						// si j'arrive a la derniere case validee
					{
						ft_putstr(answer, val_max, conds); // j'ecris la reponse
						return (1);
					}
					...
					{
						box++; // je vais a la prochaine case
						i = answer[val_max + 1][box];
						break ;
					}
				}
			}
			if (i < 0) // si j'ai tester toutes les valeurs sans succes
			{
				answer[val_max + 1][box] = val_max;
				answer[0][box] = 0;
				box--;
				answer[0][box] = 0;
				i = answer[val_max + 1][box] - 1;
			}
			if (box == -1)
				return (0);
		}
...
void	init_answer(int **answer, int val_max, int *conds)
{
	...
	while (i <= val_max)
	{
		...
		{
			if (i == val_max - 1 && ((j / val_max == 1 && up_line_conds(j,
							val_max, conds) == 2) || (j % val_max == 1
						&& left_col_conds(j, val_max, conds) == 2) || (j
						% val_max == val_max - 2 && right_col_conds(j, val_max,
							conds) == 2) || (j / val_max == val_max - 2
						&& low_line_conds(j, val_max, conds) == 2)))
				answer[i][j++] = 0;
			else
				answer[i][j++] = i;
		}
...
int	rush01(char **av, int val_max)
{
	...
	answer = malloc(sizeof(int *) * (val_max + 2));
	if (!conds || !answer)
		return (0);
	while (i <= val_max + 1)
		answer[i++] = malloc(sizeof(int) * (val_max * val_max));
	ft_write_cond(conds, av, val_max);   // j'assigne ma grille de conds
	init_answer(answer, val_max, conds);
		// j'alloue et je remplis mes grille de reponses
	if (cond_peers_validity(conds, val_max))
		i = ft_recursive_verif(answer, val_max, conds);
			// je verifie les valeurs
	else
		error_peers();
