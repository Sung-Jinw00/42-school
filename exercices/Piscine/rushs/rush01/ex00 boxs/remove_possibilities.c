...
int	comp_possibilities(int **answer, int val_max, int i, int j)
{
	...
	k = 0;
	while (k <= val_max)
		// je compare toutes les valeurs affichables des 2 cases
	{
		if (answer[k][i] != answer[k][j]) // si c'est different je sors
			break ;
		k++;
	}
	if (k - 1 == val_max) // si je suis arriver a val_max + 1,
		c'est que j'ai les memes valeurs
		return (1);
	return (0);
}
...
void	shorten_possibilities_line(int **answer, int val_max, int box,
		int vals_to_keep, int *changes)
{
	...
	while (start_line < end_line)
		// tant que je suis pas arriver a la fin de ma ligne
	{
		...
		{
			if (answer[k][start_line] != 0                         
				// si la valeur n'est pas zero
				&& answer[k][start_line] == answer[k][vals_to_keep]
					// et que ma valeur est egal a une de celles que je dois garder
				&& start_line != vals_to_keep)                     
...
int	compare_same_nbs_line(int **answer, int val_max, int box, int *changes)
{
		...
		// tant que je suis pas arriver a la fin de ma ligne
	{
		while (cmp < stop_line)
			// tant que je suis pas arriver a la fin de ma ligne
		{
			if (answer[val_max + 2][start_line] == answer[val_max + 2][cmp])
				if (comp_possibilities(answer, val_max, start_line, cmp))
					// si j'ai le meme nombre de tours affichables
					compteur++;                                          
						// j'ai un doublon
			if (compteur == answer[val_max + 2][start_line])             
				// si j'ai suffisament de cases pour afficher toutes les potentielles tours
			{
				shorten_possibilities_line(answer, val_max, box, compteur,
...
void	if_same_vals_line(int **answer, int val_max, int box, int *changes)
{
	...
	answer[val_max + 2] = malloc(sizeof(int) * val_max);
		// je malloc un tableau pour une ligne
	start_line = (box / val_max) * val_max;             
		// je vais au debut de la ligne
	stop_line = start_line + val_max;                   
		// je marque la fin de ma ligne
	while (start_line < stop_line)                      
		// tant que je suis pas arriver a la fin de ma ligne
	{
		while (k > 0)
		...
		answer[val_max + 2][start_line % val_max] = nb;
			// je met le nombre de valeurs affichables dans la case de ma ligne correspondante
		start_line++;
		k = val_max;
	}
