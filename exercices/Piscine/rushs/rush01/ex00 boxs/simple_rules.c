...
void	two_and_one_in_cond(int **answer, int val_max, int box, int *changes,
		int *conditions)
{
	int	new_nb;
	int	trigger;
	...
	new_nb = 0;
	trigger = 0;
	if (up_border(box, val_max) && answer[0][box] == val_max
		&& low_line_conds(box, val_max, conditions) == 2 && answer[0][box
		+ (val_max * (val_max - 1))] != val_max - 1)
		// si je suis sur la bordure du haut et que j'ai un 2 en conditions du bas
	{
		new_nb = box + (val_max * (val_max - 1));
		trigger++;
	}
	if (left_border(box, val_max) && answer[0][box] == val_max
		&& right_col_conds(box, val_max, conditions) == 2 && answer[0][box
		+ val_max - 1] != val_max - 1)
		// si je suis sur la bordure de gauche et que j'ai un 2 en conditions de droite
	{
		new_nb = box + val_max - 1;
		trigger++;
	}
	if (right_border(box, val_max) && answer[0][box] == val_max
		&& left_col_conds(box, val_max, conditions) == 2 && answer[0][box
		- (val_max - 1)] != val_max - 1)
		// si je suis sur la bordure de droite et que j'ai un 2 en conditions de gauche
	{
		new_nb = box - (val_max - 1);
		trigger++;
	}
	if (low_border(box, val_max) && answer[0][box] == val_max
		&& up_line_conds(box, val_max, conditions) == 2 && answer[0][box
		- (val_max * (val_max - 1))] != val_max - 1)
		// si je suis sur la bordure du bas et que j'ai un 2 en conditions du haut
	{
		new_nb = box - (val_max * (val_max - 1));
		trigger++;
	}
	if (trigger == 1)
	{
		answer[0][new_nb] = val_max - 1; // je met ma val_max
			-1 en bas pour remplir la condition
		remove_invalid_values(answer, val_max, new_nb);
		(*changes)++;
	}
}

void	vmax_in_cond(int **answer, int val_max, int box, int *changes,
		int *conditions)
{
	if (up_line_conds(box, val_max, conditions) == val_max
		|| low_line_conds(box, val_max, conditions) == val_max)
		// si j'ai val_max en condition en haut ou en bas
	{
		if (up_line_conds(box, val_max, conditions) == val_max)
	...
	if (right_col_conds(box, val_max, conditions) == val_max
		...
		// si j'ai val_max en condition de droite ou de gauche
	{
		if (right_col_conds(box, val_max, conditions) == val_max)
...
void	one_in_cond2(int **answer, int val_max, int *box, int *changes,
		int *conditions)
{
	if ((low_line_conds(*box, val_max, conditions) == 1 && *box < val_max
			&& answer[0][*box + val_max * (val_max - 1)] != val_max)
		|| (right_col_conds(*box, val_max, conditions) == 1 && left_border(*box,
				val_max) && answer[0][*box + (val_max - 1)] != val_max))
	{
	...
	else if ((up_line_conds(*box, val_max, conditions) + low_line_conds(*box,
				val_max, conditions) == val_max + 1 && up_border(*box, val_max)
			&& answer[0][*box + val_max * (up_line_conds(*box, val_max,
					conditions) - 1)] != val_max) || (left_col_conds(*box,
				val_max, conditions) + right_col_conds(*box, val_max,
				conditions) == val_max + 1 && left_border(*box, val_max)
			&& answer[0][*box + left_col_conds(*box, val_max, conditions)
			- 1] != val_max))
	{
...
void	one_in_cond(int **answer, int val_max, int box, int *changes,
		int *conditions)
{
	...
	else if (left_col_conds(box, val_max, conditions) == 1 && left_border(box,
			val_max) && (answer[0][box] != val_max))
	{
		answer[0][box] = val_max;
		remove_invalid_values(answer, val_max, box);
		(*changes)++;
...
int	four_is_placable(int **answer, int box, int val_max, char *da_wae)
{
	...
	end_line = box;
	if (!ft_strcmp(da_wae, "right")) // si j'ai val_max en condition du bas,
	...
	else if (!ft_strcmp(da_wae, "left")) // si j'ai val_max en condition du bas,
		j'ecris val_max...321 sur ma colonne
	{
	...
	else if (!ft_strcmp(da_wae, "up")) // si j'ai val_max en condition du bas,
		j'ecris val_max...321 sur ma colonne
	{
		while (end_line / val_max != val_max - 1)
		{
	...
	else if (!ft_strcmp(da_wae, "down")) // si j'ai val_max en condition du bas,
		j'ecris val_max...321 sur ma colonne
	{
		while (end_line / val_max != 0)
		{
...
void	two_in_cond_up(int **answer, int val_max, int box, int *changes)
{
	...
	else if (box / val_max > 1 && box / val_max != val_max - 1
		&& answer[0][box] == val_max)
	{
...
void	two_in_cond_low(int **answer, int val_max, int box, int *changes)
{
	...
	else if (box / val_max < val_max - 2 && box / val_max != 0
		&& answer[0][box] == val_max)
	{
...
void	two_in_cond_right(int **answer, int val_max, int box, int *changes)
{
	...
	else if (box % val_max < val_max - 2 && answer[0][box] == val_max && box
		% val_max != 0)
	{
...
void	two_in_cond(int **answer, int val_max, int box, int *changes,
		int *conditions)
{
