...
int cmp_dup_line(int **answer, int box, int val_max)
{
	...
	{
		while (same_line(box, val_max, compare) && compare >= 0)//tant qu'on est pas a la fin de la ligne et que compare est pas en dessous de zero
		{
...
int start_check(int *conditions, int **answer, int box, int val_max)
{
	if (cmp_dup_line(answer, box, val_max))//on verifie si on a un doublon
		return (0);
	if (!visible_towers_in_line(answer, box, val_max, conditions)//si j'affiche pas le bon nombre de tours
		|| !visible_towers_in_col(answer, box, val_max, conditions))//en lignes et en colonnes
		return (0);
	if (!rev_col_check(conditions, answer, box, val_max)//si je peux theoriquement afficher suffisament
		|| !rev_line_check(conditions, answer, box, val_max))//de tours dans l'autre sens
		return (0);
	if ((low_border(box, val_max) && !visible_rev_towers_in_col(answer, box, val_max, conditions))//idem dans l'autre sens
		|| (right_border(box, val_max) && !visible_rev_towers_in_line(answer, box, val_max, conditions)))
		return (0);
