...
int visible_rev_towers_in_col(int **answer, int box, int val_max, int *conditions)
{
	...
	rev_col = box - val_max;
	compteur = 1;
	while (answer[0][box] != val_max && rev_col / val_max >= 0)//tant que je suis pas arrive a 9
	{
		if (answer[0][box] < answer[0][rev_col])//si j'ai une tour plus grande que celle de depart
		{
...
int visible_rev_towers_in_line(int **answer, int box, int val_max, int *conditions)
{
	...
	rev_line = box - 1;
	compteur = 1;
	while (answer[0][box] != val_max && rev_line % val_max >= 0)//tant que je suis pas arrive a 9
	{
		if (answer[0][box] < answer[0][rev_line])//si j'ai une tour plus grande que celle du depart
		{
...
int visible_towers_in_col(int **answer, int box, int val_max, int *conditions)
{
	...
	start_col = box - ((box / val_max) * val_max);//pos actuelle - ecart du debut de colonne
	j = start_col + val_max;
	compteur = 1;
	while (answer[0][j] != 0 && j < val_max * val_max
		&& answer[0][start_col] != val_max)//tant que je suis pas revenu a ma box
	{
	...
	if (answer[0][start_col] != val_max
		...
		return(1);
	else if (answer[0][start_col] == val_max
		&& compteur == up_line_conds(box, val_max, conditions))
		return (1);
	return (0);
}
...
int visible_towers_in_line(int **answer, int box, int val_max, int * conditions)
{
	...
	start_line = box - (box % val_max);//pos actuelle - ecart du debut de ligne
	j = start_line + 1;
	compteur = 1;
	while (answer[0][j] != 0 && j < (box - (box % val_max) + val_max)
		&& answer[0][start_line] != val_max)//tant que je suis pas revenu a ma box
	{
	...
	if (answer[0][start_line] != val_max
		...
		return(1);
	else if (answer[0][start_line] == val_max
		&& compteur == left_col_conds(box, val_max, conditions))
		return (1);
	return (0);
}
