...
static int	init_rules(t_rules *rules, char **ag)
{
	...
	rules->demography = ft_atoi_philo(ag[1]);
	rules->t2die = ft_atoi_philo(ag[2]);
	rules->t2eat = ft_atoi_philo(ag[3]);
	rules->t2sleep = ft_atoi_philo(ag[4]);
	rules->max_iter = -1;
	if (ag[5])
	{
		rules->nb_of_meals = 1;
		rules->max_iter = ft_atoi_philo(ag[5]);
	}
	sem = -1;
