...
/*   Updated: 2024/11/13 20:06:46 by locagnio         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_string_line3(int j, int i, t_struct v, va_list args)
{
...
void	ft_string_line2(int j, int i, t_struct v, va_list args)
{
	int	j;
	...
	if (v.str[i] == 's')//si c'est une chaine de characteres
	{
		...
		ft_string_line3(j, i, v, args);
}


void	ft_string_line(int j, int i, t_struct v, va_list args)
{
	int	j;
	long long value;
	...
	if (v.str[i] == 'X' || v.str[i] == 'x')//si je veux un nombre en hexadecimal
	{
		...
		ft_string2_line(j, i, v, args);
}

void	field_width_line2(int j, int i, t_struct v, va_list args)
{
	long value;
	...
	if (v.str[j] == 'u')//si c'est un entier non signé
	{
		...
		ft_string_line(j, i, v, args);
}

void	field_width_line(int j, int i, t_struct v, va_list args)
{
...
void	ft_bonus_line(int i, int *count, t_struct v, va_list args)
{
	...
	if (v.str[j] >= '0' && v.str[j] <= '9' && v.str[i] != '0')//si j'ai que des chiffres
	{
		...
		*count += j;//j'ajoute le nombre de caracteres que je vais ecrire au compteur
		field_width_line(j, i, v, args);//j'ecris et compte en fonction du type
	}
}
//ex : printf("|%-10d|\n", 42)     =     "|42        |"
